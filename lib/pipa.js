	...
	getRouteFromDirectory: function (path, dir, cb) {
		...
		fs.readdir(path, function (err, files) {
			...
			files.forEach(function (file) {
				if (file && file.match(/(.js|.json)/gi)) (dir) ? routes[file.replace(/(.js|.json)/gi, '').toLowerCase()] = require(path + '/' + file) : routes[file.replace(/(.js|.json)/gi, '').toLowerCase()] = require(self.routeFilePath(file)) ; 
				else dirs.push(file);
			});
