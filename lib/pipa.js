	...
	getRouteFromDirectory: function (path, dir, cb) {
		...
		fs.readdir(path, function (err, files) {
			...
			files.forEach(function (file) {
				if (file && file.match('.json')) (dir) ? routes[file.replace('.json', '').toLowerCase()] = require(path + '/' + file) : routes[file.replace('.json', '').toLowerCase()] = require(self.routeFilePath(file)) ; 
				else dirs.push(file);
			});
